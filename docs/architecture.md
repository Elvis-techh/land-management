...
Lindero is a modular monolith: one responsive frontend, one backend API, and one
database. This is easier to deploy and audit than microservices while preserving
...
### Database: SQLite, deliberately, for now

The original plan named PostgreSQL. The implementation is **SQLite** — one file
on the server's disk — and that is a considered choice for this business, not a
shortcut:

- The office is a single location with a handful of users. The write volume is a
  few payments an hour, not a few thousand a second.
- One file is a backup a non-technical owner can understand, copy to a USB stick,
  and restore. A managed Postgres instance is one more thing to run, secure, and
  pay for.
- The schema was written to port later without a rewrite: text UUIDs generated by
  the application, ISO-8601 timestamps, integer centavos, no database-specific
  types. `drizzle-orm` speaks both dialects.

The cost of that choice, stated plainly so it is a decision rather than a
surprise:

- **One process, one machine.** The gapless receipt sequence is allocated under
  SQLite's write lock, the exchange-rate refresh runs on an in-process timer, and
...
Moving to Postgres is a migration when write concurrency or a second app server
actually becomes the constraint — not before.
