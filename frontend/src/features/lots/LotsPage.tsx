...
import { IconArchive, IconEdit } from "../../components/Icons";
...
export function LotsPage({
  ...
  // point of this approach: filter and sort are a VIEW of `lots`, not an
  // edit of it.
  const visibleLots = useMemo(
    () => sortLots(filterLots(lots, filters), sort, customersById),
    [lots, filters, sort, customersById],
  );
  ...
  const projectNames = useMemo(
    () => [...new Set(lots.map((lot) => lot.projectName))].sort((a, b) => a.localeCompare(b, "es")),
    [lots],
  );
          ...
          <table>
            ...
            <tbody>
              {visibleLots.map((lot) => {
                ...
                const holder = lot.holding ? customersById.get(lot.holding.customerId) : undefined;
                // Each row is written in ITS OWN project's unit: a project sold
                // in manzanas reads in manzanas even next to one sold in metres.
                  ...
                  <tr key={lot.id}>
                    ...
                    <td>
                      {showActions ? (
                        ...
                        <span className="row-actions-locked" title="Requiere permisos de supervisor">
                          —
                        </span>
                      )}
                    </td>
